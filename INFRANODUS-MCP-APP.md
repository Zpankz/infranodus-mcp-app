...
|---|---|---|
| `dotGraphFromText` | `generate_knowledge_graph` | ✓ |
| `graphAndStatements` | `get_graph` | ✓ |
| `listGraphs` | `list_graphs` | — |
| `graphAndAdvice` | `analyze_graph` | ✓ |
| `graphAiAdvice` | `generate_content_gaps` | — |
| `compareGraphs` | `compare_graphs` | — |
| `search` | `search_knowledge` | — |
| `importGoogleSearch` | `import_google_search` | ✓ |

### Auth

Bearer token via `INFRANODUS_API_TOKEN` env var.
...
```bash
npm install
npm run build:ui    # Vite → dist/ui/index.html (single-file)
