...
The InfraNodus MCP server (`Zpankz/mcp-server-infranodus`) declares **24 tools**
across seven capability groups. Each tool returns an
`application/vnd.mcp.app+html` content block plus a `meta.surface` hint so the
app routes its result to the appropriate UI. The visual treatment of each
result is specified in `docs/mockups/tools.html` — one card per tool, grouped
by capability.

#### Analysis — text ↔ graph

| Tool | Renders into | Card |
|---|---|---|
| `generate_knowledge_graph` | 01 Graph canvas | Full force-graph + cluster legend |
| `analyze_existing_graph_by_name` | 01 Graph canvas | Graph + stat sidecar (modularity, entropy) |
| `create_knowledge_graph` | inline + 04 | Success card with InfraNodus deep-link |
| `retrieve_from_knowledge_base` | inline (GraphRAG) | Ranked statement list with relevance score |
| `develop_text_tool` | 03 (pipeline view) | Progress checklist with per-step timing |

#### Insight — graph → meaning

| Tool | Renders into | Card |
|---|---|---|
| `generate_content_gaps` | 03 Insights & gaps | Cluster-pair cards + suggested bridge concept |
| `generate_topical_clusters` | 03 (Topics tab) | Color-keyed cluster list + keywords + pagerank bar |
| `generate_contextual_hint` | inline (above prompt) | Single AI summary + copyable prompt fragment |
| `develop_conceptual_bridges` | 03 (Bridges tab) | Bridge-concept list with rationale |
| `develop_latent_topics` | 03 (Topics tab) | Latent-topic cards with elaboration prompts |

#### Research — questions, ideas, grounded responses

| Tool | Renders into | Card |
|---|---|---|
| `generate_research_questions` | 03 (Follow-ups) | Numbered question list |
| `generate_research_ideas` | 03 (Ideas tab) | Titled idea cards with rationale |
| `research_questions_from_graph` | 03 (Follow-ups) | Question list keyed to saved graph |
| `generate_responses_from_graph` | inline (chat) | AI prose with statement-citation chips |

#### Compare — overlap & difference

| Tool | Renders into | Card |
|---|---|---|
| `overlap_between_texts` | 03 (Compare tab) | A · A∩B · B triptych of mini-graphs |
| `difference_between_texts` | 03 (Compare tab) | Diff graph + ranked unique-concept list |

#### SEO — search supply vs demand

| Tool | Renders into | Card |
|---|---|---|
| `analyze_google_search_results` | 01 Graph canvas (SERP-tinted) | Force-graph of SERP concepts |
| `analyze_related_search_queries` | 01 Graph canvas (PAA-tinted) | Force-graph of related queries |
| `search_queries_vs_search_results` | 03 (SEO tab) | Demand-only / Supply-only split list |
| `generate_seo_report` | 03 (SEO report) | 6-up coverage stats + top recommendation |

#### Memory — entities & relations

| Tool | Renders into | Card |
|---|---|---|
| `memory_add_relations` | inline | Source text with `[[wikilink]]` highlighting + extracted triples |
| `memory_get_relations` | 01 Graph canvas (ego mode) | 1-hop ego network around the queried entity |

#### Search — find & fetch

| Tool | Renders into | Card |
|---|---|---|
| `search` | 04 Resource browser (filtered) | Matching graph rows with snippet excerpts |
| `fetch` | 01 Graph canvas + statements pane | Full graph + statement list (ChatGPT Deep Research-compatible) |

### 12.1.1 Tool-result card grammar

Every tool result, regardless of surface, follows the same card grammar:

1. **Header** — color-coded dot + tool name in mono + status pill (`ok` /
   `running` / `err`) + group tag + timing.
2. **Args block** — compact `key: value` rows in mono. Strings get
   `--c-cyan`, numbers `--c-lime`, booleans/enums `--c-orange`, special tokens
   like `[[wikilinks]]` get the accent.
3. **Preview header** — uppercase mono label + badge with the most useful
   metric ("3 found · sorted by strength").
4. **Preview body** — the diagnostic visual: a graph, a ranked list, a stat
   grid, or an AI-tinted prose card. Choice is per-tool but draws from the
   same primitives (`ForceGraph`, `MiniBar`, `Stat`, `Section`).

This grammar means a host can render any tool result and the user reads the
same pattern every time — only the body shape changes.
